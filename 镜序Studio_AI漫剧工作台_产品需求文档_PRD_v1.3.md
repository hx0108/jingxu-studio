...
| V1 机器契约 | `镜序Studio_V1_ScriptStageOutput.schema.json`、`镜序Studio_V1_ShotContract.schema.json`、`镜序Studio_V1_EpisodeStoryboardExport.schema.json` |
...
- NARRATION_FIRST 的 speaker_id 固定为 `narrator`；WEAK_LIP_SYNC 和 PRECISE_LIP_SYNC 必须引用当前 StoryBible 中的 `char_*`；SUBTITLE_ONLY 可以引用 `char_*` 或为空。
...
人工可以覆盖 `WARN` 和 `INFO`，但必须保存覆盖时间和可选原因；结构错误与已确认的能力硬冲突不能直接覆盖。预计成本使用本地版本化参考价格表，只显示区间并标注“参考估算，非真实实扣”。
...
单个 JSON Schema 不能证明跨镜头引用、顺序和项目对象真实存在。整集进入 READY、导出或重新导入时，必须在同一事务中执行 Episode 级校验器：

1. ExportEnvelope 的 project_id、episode_id、episode_version 和 FormatProfile 快照存在且唯一。
2. 只包含 `lifecycle_status=ACTIVE` 且 version status=READY 的 Shot；shot_id、version_id 和 sequence 在整集内唯一。
...
7. character_ids、scene_id、prop_ids 和 `char_*` speaker_id 必须能在当前 StoryBible 版本中解析；`narrator` 是系统保留值；V1 asset_version_ids 必须为空。
8. target_duration_sec 总和偏离项目 60 至 120 秒目标时输出 WARN；用户确认后可以导出，但记录偏离原因。
9. 拆分、合并、排序、删除和恢复完成后立即重新运行集合校验；失败则整次编辑事务回滚。

V1 EpisodeStoryboardExport 只导出当前 ACTIVE 镜头版本，`lineage_completeness=CURRENT_ONLY`，不包含完整历史版本。导入空项目时，镜头聚合默认创建为 ACTIVE；`parent_version_id` 作为外部血缘元数据保留但标记 `EXTERNAL_UNRESOLVED`，不伪装为已验证本地父链。只有导回原本地项目且父版本仍存在时，才验证“同 shot 的直接前一版本”。完整历史备份和跨项目分支导入不属于 V1。

应用必须提供内置离线 Schema Registry，将三份 `$id` 映射到随应用发布的本地 Schema 文件；校验过程不得依赖访问 `jingxu.studio` 域名。离线断网环境下通过三份 Schema 和 Envelope 引用解析是 P0 Contract Test。
...
- AI 生成、人工保存、拆分、合并和恢复都创建新版本，不覆盖历史版本。
...
- 普通新建镜头的 derived_from_shot_ids 为空；拆分后的每个新镜头必须有且只有 1 个来源 shot_id；合并后的新镜头必须至少有 2 个来源且不得重复。血缘只记录 Shot 聚合 ID，不代替版本 parent 链。
...
| AC-V1-03 分镜编辑与恢复 | 已有一组 READY ShotContract | 拆分、合并、排序、删除、恢复并在断网环境导出/导入 | ID、sequence、parent_version、derived_from_shot_ids 和 Shot lifecycle_status 符合 9.7.1、9.8；离线 Registry、Envelope 和集合校验通过 |
...
| 对象 | 关键字段 |
|---|---|
| Project | id、name、genre、style、budget、dialogue_render_mode、deployment_mode、data_location |
| FormatProfile | id、aspect_ratio、width、height、fps、language、subtitle_safe_area、target_platform |
...
| ShotContractVersion | version_id、schema_version、contract_version、status（DRAFT/READY/STALE_INPUT）、parent_version_id、derived_from_shot_ids、provenance、narrative、camera、content、dialogue、continuity、generation、acceptance、locked_paths |
...
2. 落地三份 V1 Schema、Episode 集合校验器、JSON 导入导出和各阶段合法/非法 Fixture。
...
7. 实现分镜表格/卡片编辑、拆分、合并、排序和 Markdown/JSON 往返导出。
...
- 后续 TECH_DESIGN：存储、接口、ERD、任务编排、密钥和部署细节。
