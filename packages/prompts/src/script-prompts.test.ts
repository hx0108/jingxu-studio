...
describe('script prompts', () => {
  it('条件—五阶段清单—模板 ID 按阶段锁定版本、Candidate Schema 锁定 v1', () => {
    ...
    expect(
      SCRIPT_PROMPT_MANIFEST.map(({ promptTemplateId }) => promptTemplateId).sort(),
    ).toEqual([
      'beat_sheet/v2',
      'concept/v2',
      'episode_outline/v2',
  ...
  it('条件—阶段模板未列出该阶段全部必填字段—候选必被校验拒绝，模板必须逐一列出', () => {
    const requiredFields: Readonly<Record<'CONCEPT' | 'STORY_BIBLE' | 'EPISODE_OUTLINE' | 'BEAT_SHEET' | 'SCENE_SCRIPT', readonly string[]>> = {
      BEAT_SHEET: ['beats', 'beat_id', 'sequence', 'purpose', 'description', 'estimated_duration_sec'],
      CONCEPT: ['title', 'genre', 'target_audience', 'core_conflict', 'theme', 'synopsis'],
      EPISODE_OUTLINE: ['episode_goal', 'opening', 'midpoint', 'climax', 'ending_hook', 'target_duration_sec'],
      SCENE_SCRIPT: ['scenes', 'script_scene_id', 'sequence', 'scene_id', 'character_ids', 'action', 'spoken_lines', 'speaker_id', 'line_type', 'text', 'estimated_duration_sec'],
      STORY_BIBLE: ['characters', 'world_rules', 'scenes', 'props', 'name', 'appearance', 'personality', 'motivation', 'description'],
    };
