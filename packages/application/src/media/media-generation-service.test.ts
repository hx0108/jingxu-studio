...
/** 内存版 MediaRepository：只为驱动 service 语义，不模拟 SQL 约束之外的并发细节。 */
class InMemoryMediaRepository implements MediaRepository {
  ...
  public completeCandidateSucceeded(): Promise<MediaCandidateRecord> {
    throw new Error('not used in service tests');
  }

  public completeCandidateFailed(): Promise<MediaCandidateRecord> {
    throw new Error('not used in service tests');
  }
