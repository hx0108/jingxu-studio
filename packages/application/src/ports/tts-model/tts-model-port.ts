 ...
 *
 * Application 只依赖该接口与归一化类型；Ark 请求形状、HTTP header、
 * Authorization 或 Provider 专有错误只允许存在于实现该 Port 的 Adapter
 * （ArkTtsModelAdapter / MockTtsModelAdapter）。同项目严格串行、两段式证据、
...
export interface TtsModelPort {
  ...
  /**
   * 同步合成一次配音：调用返回即终态，音频字节随结果返回（无结果 URL，
   * 不存在 download 段）。signal 取消 SHALL 归一化为 MODEL_CANCELLED。
