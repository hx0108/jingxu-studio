...
分镜工作台 MUST 在整集当前版本 status=READY 时提供「导出整集」入口；非 READY 时 MUST 不渲染该入口。Σ 软带外首次导出被拒时 UI MUST 呈现确认对话（展示实际 Σ）与原因输入，用户确认后 MUST 以同命令携带 warnConfirmed 与 deviationReason 重发。导出成功 MUST 向用户呈现成功反馈；失败 MUST 呈现稳定错误码对应的人话提示。

#### Scenario: READY 出现入口、导出往返成功

- **GIVEN** 整集 READY
- **WHEN** 用户进入分镜工作台并点击「导出整集」、完成落盘
- **THEN** UI SHALL 呈现导出成功反馈（含 export_id 或文件哈希末四位等非路径标识）

#### Scenario: 非 READY 无入口

- **GIVEN** 整集当前版本 DRAFT
- **WHEN** 用户进入分镜工作台
- **THEN** 「导出整集」入口 SHALL 不存在
