...
- [x] 3.3 实现 `startDemo` 原子 Seed、Fixture/CAS staging 和演示恢复逻辑，所有演示任务固定 Mock 并持续保存可查询的演示标识。（注：媒体改为用户触发 + 调度器 resolveModel 按项目路由 Mock，替代种子预置 CAS；演示标识 = projects.experience_mode + 进程内登记）
