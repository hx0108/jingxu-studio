...
- 派生常量：`dialogue.dialogue_mode_source='PROJECT_DEFAULT'`、`override_reason=null`（V1 无项目级口型默认值可偏离）、`audio_required`/`lip_sync_required` 由 `(spoken_text, dialogue_render_mode)` 确定性推导、`speaker_id`/`estimated_speech_duration_sec` 同样由系统按 allOf 条件矩阵派生——无台词（`spoken_text` 为空）时恒 `null`/`0`，NARRATION_FIRST 且有台词时 speaker 恒 `narrator`（schema allOf 已规定精确值，系统推导保证一次通过，不让模型猜；真实 Qwen 联调曾因模型对无台词镜头给非空值被 FINAL 层 `SCHEMA_VALIDATION_CONST/TYPE` 拒绝，回归钉见 `shot-system-fields.test.ts`）、`continuity.asset_version_ids=[]`、`continuity.previous_shot_id` 系统派生（`CONTINUOUS_ACTION` 且非首镜头 → `sequence-1` 镜头的 `shot_id`，否则 null；首镜头 `CONTINUOUS_ACTION` 派生 null，由集合校验层先行拦下）、`generation_constraints.budget_estimate=UNKNOWN 变体`（min/max/price_version 均 null）、`acceptance.human_review_required=true`、`locked_paths=[]`。
