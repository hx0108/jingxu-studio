...
- 新增 `JobService` 与 `job create/get/list/cancel/retry` IPC 契约/Host、`events.subscribeJobUpdates`：Command 携带 `requestId`、修改命令携带 `expectedVersionId`，复用既有启动写门；本 Change 只交付通用 Job 基座和安全的 `JOB_SUBMISSION_UNAVAILABLE` 降级，具体阶段的提交、运行与恢复重校验由 `staged-script-generation` 注入后才开放生产写入。页面切换或 Renderer 刷新不取消已持久化 Job（AGENTS.md §11.1、§14）。
...
- 接入启动状态机：通用 Job/Provider Host 只在 `READY` 后构造，`READ_ONLY_FAULT` 期间不构造写路径且 `job`/`provider` 写命令返回既有 `STARTUP_WRITE_BLOCKED`；具体阶段 JobRunner 调度与可提交恢复在 `staged-script-generation` 注入业务 seam 后激活。
...
- **IPC/UI**：新增 `job create/get/list/cancel/retry` 的类型与 Host 边界、`provider getProfile/saveProfile/saveCredential/testCredential/deleteCredential` 与 `events.subscribeJobUpdates`；统一 `AppError`，Renderer 不接收 API Key、Authorization header 或未脱敏 Provider 错误。由于本 Change 不提供具体阶段提交器，Job 写命令在 READY 后返回稳定 `JOB_SUBMISSION_UNAVAILABLE`/`JOB_NOT_CANCELLABLE`，不得伪造成功；`staged-script-generation` 接入后才成为可用生产入口。
...
- **测试与发布**：增加 Unit（状态机、幂等键、错误归一化、退避）、Provider Contract（Mock 全矩阵）、Repository Integration（领取原子性、证据回滚、崩溃恢复矩阵）、Electron E2E（启动门与安全边界）与 Windows x64 packaged smoke。本 Change 的自动化验收为 **AC-V1-04 Mock 全矩阵 + 凭据安全/连通性基线**；真实 Qwen `testCredential` 需要用户自备凭据与明确联网操作，记录为发布前人工检查，不作为离线 Sync/Archive 阻断，也不构成 AC-V1-01 证据。
