...
## D1: 主方案——候选层跨字段值校验（A）是否叠加单角色系统派生（B）【已拍板 2026-08-20：A+B 组合】
...
## D2: 候选层校验的强度【已拍板 2026-08-20：按推荐——非空 string + pattern `^(narrator|char_[A-Za-z0-9_-]+)$`，与 ShotContract 1.1.0 speaker_id 同正则】
...
## D3: 残余缺口显式接受【已拍板 2026-08-20：按推荐——接受，不扩 isRepairable；若未来实录出现修复后非 bible 键形态再立独立 change】
