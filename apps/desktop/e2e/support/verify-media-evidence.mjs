...
//   C. SUBMIT SUCCEEDED：blob 非空、usage 图片数=1、provider_request_id 恒 null
//      （2026-08-20 实录：真实 Seedream 同步响应无顶层 id，Mock raw 已对齐）；
  ...
  for (const row of submitRows) {
    if (row.status === 'SUCCEEDED') {
      ...
      if (row.provider_request_id !== null)
        violations.push(`SUBMIT_UNEXPECTED_REQUEST_ID:${row.id}`);
